...
by both price and location at the same time.
...
starting from the cheapest to most expensive, we can simply replace the
last line with
...
Part of a job of an analyst is to evaluate the quality and integrity of
the data, and try to work around or understand when such oddities
...
average price of 130 yen more or less equals to 1 dollar. The highest
bill in Japan is that of 10.000 Yen, which is roughly valued at around
75US\$ at the date of this analysis.
...
impressive. Now let's see the price divided by neighbourhood.

![](images/newyorkaverageprices.jpg)
...
from the analysis. A glance at the "price" column can show us that the
values seem a bit more reasonable than Tokyo if in dollars, however one
must wonder why these values are in a string format and not in numeric
in the first place.
...
![](images/parisaverageprice.jpg)

London

![](images/londonnumber.jpg)

![](images/londonavgpriceperneighbourhoodupdated.jpg)

Rio de Janeiro

![](images/rionumber.jpg)

![](images/rioaverageprice.jpg)
...
1 - The Shanghai data was empty, even looking at the txt document showed
us there is no data there. Even another data provided in the website had
the same result. Unfortunately, we are prone to faulty data in this line
of work.
